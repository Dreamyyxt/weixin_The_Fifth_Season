...
const SERVICE_TYPES = [
  { id: 7, name: '去甲（60分钟）'       },
  { id: 3, name: '嫁接睫毛（90分钟）'   },
  { id: 1, name: '日式凝胶美甲（90分钟）'},
  { id: 4, name: '种植睫毛（120分钟）'  },
  { id: 5, name: '美甲+美睫组合（180分钟）'},
  { id: 2, name: '高定款式美甲（120分钟）'},
  { id: 6, name: 'VIP高定全套（240分钟）'},
];
...
Page({
  data: {
    ...
    form: {
      ...
      serviceId: '', duration: '', durationMin: '',
    },
    saving: false,
  },
  ...
  async loadPkg(id) {
    ...
    try {
      ...
      const p = res.data;
      const idx = SERVICE_TYPES.findIndex(s => s.id == p.serviceId);
      this.setData({
        serviceTypeIndex: idx,
        form: {
          type:        p.type || 'package',
          name:        p.name || '',
          desc:        p.desc || '',
  ...
  selectServiceType(e) {
    ...
    const svc = SERVICE_TYPES[idx];
    this.setData({ serviceTypeIndex: idx, 'form.serviceId': svc ? String(svc.id) : '' });
  },
  ...
  toggleTechId(e) {
    ...
    if (selected.includes(techId)) {
      this.setData({ selectedTechIds: selected.filter(t => t !== techId) });
    } else {
      this.setData({ selectedTechIds: [...selected, techId] });
    }
  },
  ...
  async save() {
    ...
    try {
      ...
      if (form.type === 'package') {
        ...
        pkg.serviceId   = Number(form.serviceId) || null;
